...
@interface LocalCoreDataManager ()
...
static LocalCoreDataManager *coreDataManager_ = nil;

+ (LocalCoreDataManager *)sharedManager
{
    // インスタンスが無ければ格納
    if (!coreDataManager_) {
        coreDataManager_ = [LocalCoreDataManager new];
    }
...
- (NSManagedObjectModel *)managedObjectModel
{
    if (_managedObjectModel == nil) {
        NSURL *modelURL = [[NSBundle mainBundle] URLForResource:CORE_DATA_LOCAL_NAME withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator
{
    if (_persistentStoreCoordinator == nil) {
        ...
        NSURL *storeURL = [documentDirectoryURL URLByAppendingPathComponent:SQLITE_LOCAL_FILE_NAME];
        // ストアコーディネーターを生成
...
- (void)savePersistenceChange
{
    NSError *error;
    [self.persistentContext save:&error];
    if (error != nil) {
        NSLog(@"Unresolved error %@, %@", error, [error userInfo]);
    }
}
